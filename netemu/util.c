...
struct netemu_sender_udp* netemu_util_prepare_sender_on_socket_at_addr(NETLIB_SOCKET socket, netlib_sockaddr_in *addr_in, int size) {
	...
	sender = netemu_sender_udp_new_on_socket(netemu_util_copy_addr((netlib_sockaddr*)addr_in,size),socket,size);
	return sender;
}
...
netlib_sockaddr* netemu_util_copy_addr(netlib_sockaddr *addr, int addr_size) {
	netlib_sockaddr* copy;
	copy = malloc(addr_size);
	memcpy(copy,addr,addr_size);
	return copy;
}

netlib_sockaddr_in* netemu_util_create_addr(NETEMU_DWORD address, unsigned short port, int *size) {
	netlib_sockaddr_in *addr;
	addr = malloc(sizeof(netlib_sockaddr_in));
	addr->sin_addr.s_addr = address;
	addr->sin_port = port;
	addr->sin_family = NETLIB_AF_INET;
	*size = sizeof(netlib_sockaddr_in);
	return addr;
}

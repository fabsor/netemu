...
struct server_connection *netemu_server_connection_new(char* user, char* emulator_name, struct netemu_sender_buffer* buffer) {
	...
	connection->_internal->chat_callback = netemu_list_new(3, FALSE);
	connection->_internal->game_created_callback = netemu_list_new(3, FALSE);
	connection->_internal->join_callback = netemu_list_new(3, FALSE);
	connection->_internal->leave_callback = netemu_list_new(3, FALSE);
	connection->_internal->play_values_callback = netemu_list_new(3, FALSE);
	...
	connection->_internal->users = netemu_list_new(10, FALSE);
	netemu_list_register_sort_fn(connection->_internal->users,_server_connection_user_comparator);
	connection->_internal->games = netemu_list_new(10, FALSE);
